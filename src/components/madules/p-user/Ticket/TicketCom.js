...
export default function TicketCom() {
  let { cyUserID, mobile, setXtFlagSpinnerShow, setFlagMessageNotification } =
  ...
  const getThicketById = () => {
    ...
    async function myApp() {
      const res = await fetch(
        `${apiUrl}/api/CyTicket/getMessagesForTickets?TicketID=${ticketId}`,
        {
          method: "GET",
          credentials: "include",
  ...
  const closeChat = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        async function myApp() {
          const res = await fetch(
            `${apiUrl}/api/CyTicket/closeTicket?ticketId=${ticketId}`,
            {
              method: "PUT",
              credentials: "include",
...
sendTicket(obj)
            ...
            <button
              className={
                textAreaB || guIdC
                  ? `btn btn-info ${style.createbutton}`
                  : `btn btn-info ticket-disable ${style.createbutton}`
              }
            >
              ارسال پیام
            </button>
        ...
        {ticketArray?.length != 0 && (
          <div className="col">
            <div className={`table table-striped table-hover ${style.table}`}>
              ...
              <tbody>
                {ticketArray?.length != 0 &&
                  ticketArrayRevers?.map((item) => (
                    <tr>
                      <td>{item.id}</td>
                      <td>{item.title}</td>
                      <td>
                        <DateFormat dateString={`${item.openedAt}`} />
                      </td>
      ...
      <>
        ...
        >
          ...
          <Modal.Body>
            <div className="Ticket-div boxSh centerc paziresh-code">
              {stateId != 3 && (
                <div className="ticket-right-message-divB">
                  <textarea
                    name=""
                    id=""
                  ...
                  <div className="ticket-right-message-file-div">
                    <input
                      ...
                      onChange={fileChange}
                    />
                    <div className="order-file-i ticket-hide " ref={classRefD}>
                      <i
                  ...
                  getChatsRevers?.map((item) => (
                    <>
                      ...
                      >
                        ...
                        >
                          {item.description}
                          <span className="ticket-messeage-desc-sendername">
                            {item.senderName}
                          </span>
