...
{catArray.length!=0 && catArray.map((item=>(
        <SwiperSlide className={styles.swiper_slide}>
      
        <img className={styles.swiper_img_A} src={`${item.bigImg}`}
alt={`${item.cyCategoryId}`} />
          <img className={styles.swiper_img_B} src={`${item.smallImg}`} alt={`${item.cyCategoryId}`} />
