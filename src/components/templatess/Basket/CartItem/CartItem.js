...
const CartItem = (props) => {
  ...
  const alertB = () =>
    alertN(
      "center",
      "error",
      "بیش تر از یک عدد مجاز به سفارش نمی باشید...",
      1500
    );
  ...
  return (
    <>
      <tr className={`${style.tr}`}>
        ...
        {/* <!-- title --> */}
        <td
          className={` tp-cart-title ${style.title}`}
          onClick={() => setXtFlagSpinnerShow(true)}
        >
          <a
            href={
              xtFlagLogin
                ? `product/${props.cyProductID}`
                : `product/${props.id}`
            }
          >
            {props.name}
        ...
        <td className="tp-cart-action">
          ...
          >
            ...
            <button
              className={`btn btn-primary m-1 ${style.btn_product_hide}`}
              onClick={() => setXtFlagSpinnerShow(true)}
            >
          ...
          <button
            className={`btn btn-danger m-1 ${style.btn_product}`}
            onClick={(e) => {
              AlertA(handleRemove);
            }}
          >
          ...
          <button
            className={`btn btn-danger m-1 ${style.btn_product_hide}`}
            onClick={(e) => {
              AlertA(handleRemove);
            }}
          >
