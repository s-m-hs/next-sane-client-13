        ...
        <div className={`col-2  ${Styles.right_maindiv} centerc`}>
          ...
          <Accordion defaultActiveKey={["0"]} alwaysOpen>
            <Accordion.Item eventKey="0">
              <Accordion.Header className={`${Styles.AccordionCom}`}>
                لوازم جانبی
              </Accordion.Header>
              <Accordion.Body>
 ...
 {mainCatA?.childs?.length!=0 && mainCatA?.childs?.map(item=>(
  <Link href={`/category/${item.id}`}>  <span>{item.name}</span></Link>
        ...
        <div className={`col-12 col-md-10 ${Styles.category_col}`}>
            ...
            {mainCatChilds != null &&
              mainCatChilds.map((item, index) => (
                <>
                  <div
                    key={item.id}
                    className={`centerc ${Styles.category__cart_div}`}
                  >
                    <CardAButton
                      imgSrc={item.imageUrl}
                      text={item.name}
                      changeIdProp={() => changeId(item.code)}
                      code={item.code}
                    />
                    <span>{item.name} </span>
                  </div>
          ...
          <div
            className={`row row-cols-4 centerr pt-4 ${Styles.products_card} boxSh `}
          >
